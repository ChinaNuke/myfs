...
int search_inode_addr(char* name,vdisk_handle_t handle,uint32_t block,uint32_t blocksize,uint8_t* site)
{
    ...
    for(uint8_t i=0;i<dir_num;i++){
        if(dir[i].inode_addr==0){    /*blcok的一个目录项大小的空间可用，应该是NULL还是inode_addr为0 删除操作是将inode_addr置0*/
            if(*site==0)
                *site = i;  //地址最小的blcok没存满 i这个位置是空的
        }
...
int add_document(char* name,vdisk_handle_t handle,uint32_t blocksize,uint16_t inode_id)
{
    ...
    */
     ...
     *这里将判重名文件之后添加记录项的blcok号，和block 里面的第几个位置在判重过程中被改写
    */
    uint32_t add_document_block=0;
    uint8_t site;
    
    for(uint32_t i=0;i<inode->blocks;i++){ /*遍历indoe说明的数据块个数 */
       
        uint32_t block_id = block_addr(handle,inode,i,blocksize); //找到block_id,block_addr在inode.h文件
        
        if(search_inode_addr(name,handle,block_id,blocksize,&site)!=-1){  //使用文件名找到了inode_id，添加的文件名重复了
...
int del_document(char* name,vdisk_handle_t handle,uint32_t blocksize,uint16_t inode_id)
{
    ...
    */
    ...
    for(uint32_t i=0;i<inode->blocks;i++){ /*遍历indoe说明的数据块个数 */
        ...
        if(search_inode_addr(name,handle,block_id,blocksize,&site)!=-1){  //使用文件名找到了inode_id，要删除的文件名匹配到了
            // printf("%s","文件已存在！");
            dir_entry_t* dir = block_to_dir(handle, block_id,blocksize);  //将这个block 存的目录项表读出转成目录项表
            
            for(int j=0;j<dir_num;j++){
                if(strcmp(dir[j].name,name)==0){  //文件名相同
                    //inode管理模块对该inode的释放
                    
                    dir[j].inode_addr=0;  //等于0表示该目录被删除 找空位也是这样判断
